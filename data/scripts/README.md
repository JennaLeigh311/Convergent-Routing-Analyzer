...
| 3 | Build the `cra/data-tools` client image (osm2pgrouting + python exporter) | `Dockerfile` | `docker build` |
...
- **`segment_id = "{osm_way_id}:{seq}:{dir}"`** (§1). `osm_way_id` is
  ...
  a wire key). `seq` is the 0-based ordinal of the sub-segment within its way
  after pgRouting splits it at intersections (ordered by `gid`). `dir` is `F`
...
- **`length_m >= endpoint chord` (§2 invariant).** `length_m` is computed as the
  **sum of haversine arcs over the emitted (rounded) coordinates**, using the
  ...
  straight 2-point edge it equals the loader's chord exactly. `length_m` is
...
- `Dockerfile` — `cra/data-tools` (osm2pgrouting + `postgresql-client` + python3 + psycopg2).
